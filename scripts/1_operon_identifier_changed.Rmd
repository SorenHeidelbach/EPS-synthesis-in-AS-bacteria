...
filename_psiblast <- c("cellulose1")
...
gene_height <- 4
pdf(glue("./figures/gggenes_", paste(filename_psiblast, collapse = "_"), "_min_genes{min_genes}.pdf"), 
    height = length(unique(genes$operon)), 
    width = 15)
...
```{r}
test <- psiblast_filtered %>% 
  mutate(
    in_operon = ifelse(Target_label %in% psiblast$Target_label, "yes", "no"),
    bit_perc = `Bit score` * Percent_identity / 100
  )
ggsave(filename = "test.pdf", 
ggplot(test, aes( x = Query_label, y = bit_perc, col = in_operon)) +
  geom_boxplot(outlier.colour = NA) +
  geom_jitter(alpha = 0.4, size = 0.7, position = position_jitterdodge(jitter.width = 0.2)) +
  theme_minimal() +
