...
psiblast <- psiblast %>% 
  ...
  mutate(
    ...
    end_target_plot = end_target*3 + start,
    start_target_plot = start_target*3 + start
  )
```
...
ggplot(psiblast, aes(xmin=start, xmax=end, y = ID, forward = strand)) + 
  ...
  theme_genes() +
  geom_text(data=psiblast %>% mutate(start = (start_target_plot + end_target_plot)/2), aes(x=start, label = Query_label), nudge_y = 0.3 )
dev.off()
```
