...
```{r}
filename_psiblast <- c("pnag_ica", "pnag_pga")
ips_path <- d("ips")
perc_id = 20
max_dist_prok = 8
max_dist_gene = 2000
...
psiblast <- psiblast %>% 
  ...
  left_join(y=query_metadata[, c("Genename", "Function")], by = c("Query_label" = "Genename")) %>% 
  distinct()
#rm(gff, magstats)
```
...
psiblast_surr <- psiblast %>% 
  ...
  full_join(psiblast) %>% 
  group_by(operon) %>% 
  filter(length(unique(Query_label[which(!is.na(Query_label))])) >= min_genes) %>% 
  group_by(operon) %>% 
...
```{r}
gene_height <- 4
pdf(f(paste0("/gggenes_",filename_psiblast,".pdf")), height=length(unique(psiblast_surr$operon)), width = 15)
...
dev.off()
print(length(unique(psiblast_surr$operon)))
