...
colnames(upset_df) <- colnames(upset_df) %>%  dplyr::recode(
   "cellulose1" = "Cellulose1*",
   "cellulose2" = "Cellulose2*",
   "curdlan" = "Curdlan*",
   "pnag_eps" = "PNAG (eps)",
   "pnag_ica" = "PNAG (ica)*",
   "pnag_pga" = "PNAG (pga)*",
...
metadata$group <- case_when(
  ...
  metadata$sets %in% c("Cellulose1*", "Cellulose2*")                   ~ "cellulose",
  TRUE ~ "other"
)

pdf(paste0(d.dir, "/r_output/UpSet_operon_shared_by_queries_simple_nulO3_eps_3.pdf"), width = 8, height = 12)
UpSetR::upset(
  upset_df,
  sets = order_sets, nintersects = 80,
  order.by = "freq",
